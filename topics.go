...
func (lib *LibraryNoDB) configureTopicsPrework() {
	...
	lib.topicsWork.ClearedUp = func(ctx context.Context, _ error, why topicsWhy, unprefixedTopics []string) {
		lib.tracer.Logf(ctx, "[events] prior error creating topics %v, preventing %s, has cleared up", unprefixedTopics, why.why)
	}
	lib.topicsWork.FirstWorkMessage = func(ctx context.Context, _ topicsWhy, unprefixedTopic string) {
		lib.tracer.Logf(ctx, "done waiting for topic listing to complete (%s needs to be created)", unprefixedTopic)
	}
	lib.topicsWork.NotRetryingError = func(ctx context.Context, unprefixedTopic string, why topicsWhy, err error) error {
		...
		lib.tracer.Logf(ctx, "[events] %s: %+v", why.why, err)
		return err
	}
	lib.topicsWork.RetryingOrNot = func(ctx context.Context, doCreate bool, unprefixedTopic string, why topicsWhy) {
		if doCreate {
			lib.tracer.Logf(ctx, "[events] %s: will re-attempt creation of topic %s, previous attempt failed", why.why, unprefixedTopic)
		} else {
			lib.tracer.Logf(ctx, "[events] %s: will NOT re-attempt creation of topic %s yet, previous attempt failed", why.why, unprefixedTopic)
		}
	}
	lib.topicsWork.ItemPreWork = func(ctx context.Context, unprefixedTopic string, why topicsWhy) error {
		...
		if lib.mustRegisterTopics && !ok && unprefixedTopic != heartbeatTopic.Topic() {
			lib.tracer.Logf(ctx, "[events] %s: requested topic, %s, not pre-registered", why.why, unprefixedTopic)
	...
	lib.topicsWork.ItemWork = func(ctx context.Context, unprefixedTopic string, why topicsWhy) error {
		...
		lib.tracer.Logf(ctx, "[events] %s: attempting creation of topic %s with replicas %d and min.insync %d", why, prefixedTopic, tc.ReplicationFactor, mir)
		client, err := lib.getController(ctx)
		if err == nil {
			lib.tracer.Logf(ctx, "[events] %s: making topic creation request for %v", why, prefixedTopic)
			...
			if err == nil {
				...
				switch {
				case err == nil:
					lib.tracer.Logf(ctx, "[events] %s: topic %s no error when creating", why, prefixedTopic)
				case errors.Is(err, kafka.TopicAlreadyExists):
					lib.tracer.Logf(ctx, "[events] %s: topic %s already exists", why, prefixedTopic)
					err = nil
				default:
					// uh, oh. Handled later
				}
				for tpc, topicErr := range resp.Errors {
					if tpc != prefixedTopic {
						lib.tracer.Logf(ctx, "[event] received create topic response for topic (%s) not in request (%s %s): %s", tpc, why, prefixedTopic, topicErr)
					}
				}
			}
			if resp.Throttle != 0 {
				lib.tracer.Logf(ctx, "[events] %s: topic creation request was throttled for %s", why, resp.Throttle)
			}
		}
		return err
	}
	lib.topicsWork.ItemDone = func(ctx context.Context, unprefixedTopic string, why topicsWhy) {
		lib.tracer.Logf(ctx, "[events] %s: topic %s should now exist", why.why, unprefixedTopic)
	}
	...
	lib.topicsWork.ItemPending = func(ctx context.Context, unprefixedTopic string, why topicsWhy) {
		lib.tracer.Logf(ctx, "[events] %s: will wait for creation attempt of topic %s to complete", why.why, unprefixedTopic)
	}
...
func (lib *LibraryNoDB) listAvailableTopics(ctx context.Context) {
	...
	dialer := lib.dialer()
	for {
		lib.tracer.Logf(ctx, "[events] starting over on listing topics")
		for _, i := range rand.Perm(len(lib.brokers)) {
			broker := lib.brokers[i]
			lib.tracer.Logf(ctx, "[events] connecting to %s to list topics", broker)
			conn, err := dialer.Dial("tcp", broker)
			if err != nil {
				lib.tracer.Logf(ctx, "[events] could not connect to broker %s, was going to list topics: %v", broker, err)
				continue
			}
			defer func() {
				_ = conn.Close()
			...
			if err != nil {
				lib.tracer.Logf(ctx, "[events] could not list partitions on broker %s: %v", broker, err)
				continue
			}
			lib.tracer.Logf(ctx, "[events] listing existing topics...")
			...
			for _, p := range partitions {
				...
				if lib.prefix != "" && unprefixedTopic == p.Topic {
					if debugPrefixIgnore {
						lib.tracer.Logf(ctx, "[events] topic %s found in partition, IGNORING (not prefixed)", p.Topic)
					}
					continue
				}
				lib.tracer.Logf(ctx, "[events] topic %s found in partition", unprefixedTopic)
				lib.topicsWork.SetDone(unprefixedTopic)
			}
			lib.tracer.Logf(ctx, "[events] done listing existing topics")
			return
		}
		lib.tracer.Logf(ctx, "[events] waiting before making another attempt to list topics")
	}
}
