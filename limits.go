...
func (lib *LibraryNoDB) sizeCapBrokerEffective() int64 {
	if lib.sizeCapBrokerState.Load() != sizeCapFetchFinished {
		if debugLimits {
			lib.tracer.Logf("limits: sizeCapBrokerEffective - fetch unfished, returning 0")
		}
		return 0
	}
...
func (lib *LibraryNoDB) sizeCapRunPrecreatedScan(ctx context.Context) {
	if debugLimits {
		lib.tracer.Logf("limits: sizeCapRunPrecreatedScan started")
		defer func() {
			lib.tracer.Logf("limits: sizeCapRunPrecreatedScan completed")
		}()
	}
	defer func() {
...
func (lib *LibraryNoDB) sizeCapFetchTopicConfigLimits(ctx context.Context, topics []string) {
	if debugLimits {
		lib.tracer.Logf("limits: sizeCapFetchTopicConfigLimits start %v", topics)
		defer func() {
			lib.tracer.Logf("limits: sizeCapFetchTopicConfigLimits complete %v", topics)
		}()
	}
	...
	for {
		...
		err := func() error {
			...
			for i, topic := range loadList {
				...
				tl := lib.sizeCapGetOrInit(topic)
				if result.Error != nil {
					_ = lib.RecordErrorNoWait("fetchSpecificTopicConfiguration", errors.Errorf("fetch topic (%s) config for size caps: %w", topic, result.Error))
				...
				tl.overrideMax = v
				lib.sizeCapComputeEffective(tl, topic)
				tl.state.Store(sizeCapFetchFinished)
				close(tl.ready)
				lib.tracer.Logf("[events] topic %s limit %d", topic, tl.effective)
			}
...
func (lib *LibraryNoDB) sizeCapTopic(ctx context.Context, topic string) int64 {
...
func (lib *LibraryNoDB) sizeCapComputeEffective(tl *sizeCapTopicLimit, topic string) {
	if tl.effective != 0 {
		return
	}
	...
	if debugLimits {
		lib.tracer.Logf("limits: topic %s: broker %d override max %d -> %d",
			topic, broker, tl.overrideMax, tl.effective)
	}
}
